...
- **External API** : TMDB API (애니 관련 정보)
...
| **FU-106** | 관심 애니메이션 등록 | - TMDB API를 연동하여 사용자가 좋아하는 애니메이션/캐릭터/장르 목록 등록 및 수정 (AI 추천의 입력 데이터)<br>- 필수 3개 등록 | 마이페이지 |
...
| ID | 항목 | 상세 설명 | 페이지 |
| :--- | :--- | :--- | :--- |
| **FA-201** | 굿즈 등록 | **[등록 정보]**<br>- 제목, 굿즈 관련 애니메이션 선택, 굿즈 종류(카테고리), 상품 설명, 이미지(다중 업로드) 등록<br>**[경매 조건 설정]**<br>- 시작가 (필수)<br>- 상한가/즉시구매가 (선택)<br>- 최초 경매 기간 (기본 값 2~3일 설정, 최댓값 14일) | 굿즈 등록 페이지 |
...
| ID | 항목 | 상세 설명 | 페이지 |
| :--- | :--- | :--- | :--- |
| **FE-601** | [AI] 맞춤형 굿즈 추천<br>(Vector Search) | **[유사도 기반 추천 로직]**<br>1. 사용자 관심 애니(3개) 벡터 평균/병합 -> User Vector 생성<br>2. 전체 애니메이션 벡터와 User Vector 간 코사인 유사도 계산<br>3. 유사도 높은 Top N개 애니메이션 도출 및 관련 굿즈 노출 | 메인페이지<br>(추천 영역) |
| **FE-602** | [User] 관심 애니 설정<br>및 벡터 동기화 | 1. TMDB API 통해 메타데이터(장르, 개요 등) 확보<br>2. DB에 벡터값 없는 경우 FastAPI(Gemini) 호출 -> 임베딩 생성 및 Vector DB 저장<br>3. 유저 프로필에 관심 애니 ID 3개 매핑 저장 | 마이페이지/<br>초기설정 |
| **FE-603** | [Admin] 굿즈 등록 시<br>AI 임베딩 자동 생성 | **[데이터 파이프라인]**<br>1. 신규 굿즈 등록 시 연관 애니 정보 TMDB 조회<br>2. 줄거리/장르 텍스트를 768차원 벡터로 변환(Embedding) -> Vector DB 적재<br>3. 추천 시스템 검색 대상으로 즉시 반영 | 굿즈 등록 페이지 |
| **FE-604** | 인기/트렌드 노출 | **[통계 기반 추천]**<br>- 사용자들의 찜(Like) 수, 조회수 등을 집계하여 실시간 급상승 인기 굿즈 노출 | 메인페이지 |
